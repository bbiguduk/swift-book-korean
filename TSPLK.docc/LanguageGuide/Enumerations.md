...
열거형은 케이스 초기 값을 제공하기 위해 초기화 구문을 정의할 수도 있습니다;
...
### 암시적으로 할당된 원시 값 (Implicitly Assigned Raw Values)
...
이 초기화 구문을 사용하여 열거형의 새 인스턴스를 생성할 수 있습니다.
...
이러한 점 때문에, 원시 값 초기화 구문은 항상 *옵셔널* 열거형 케이스를 반환합니다.
위의 예제에서 `possiblePlanet`은 `Planet?` 타입이나
"옵셔널 `Planet`" 타입입니다.

> Note: 원시 값 초기화 구문은 모든 원시 값을 열거형 케이스로 반환할 수 없으므로,
> 실패 가능 초기화 구문입니다.
> 자세한 내용은 <doc:Initialization#실패-가능-초기화-구문-Failable-Initializers>을 참고 바랍니다.

`11`의 위치로 행성을 찾는다면,
원시 값 초기화 구문에서 반환된 옵셔널 `Planet` 값은 `nil`입니다:
