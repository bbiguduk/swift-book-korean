...
<doc:Generics#Where-절이-있는-제너릭-확장-Extensions-with-a-Generic-Where-Clause>에 설명된대로
제너릭 타입을 확장하여 조건부로 기능을 추가할 수도 있습니다.
...
확장은 기존 타입에 연산 인스턴스 프로퍼티와 연산 타입 프로퍼티를 추가할 수 있습니다.
이 예제는 거리 단위 작업에 필요한
Swift `Double` 타입의 다섯 개의 연산 인스턴스 프로퍼티를 추가합니다:
...
부동 소수점 값에 점 문법을 사용할 수 있습니다.

이 예제에서 `1.0`의 `Double` 값은 "1 미터"로 표시됩니다.
...
아래의 예제는 사각형을 나타내기 위해 커스텀 `Rect` 구조체를 정의합니다.
이 예제는 모든 프로퍼티에 `0.0`의 기본 값을 제공하는
...
`Rect` 구조체는 모든 프로퍼티에 대해 기본 값을 제공하므로
...
아래 예제는 `Int` 타입으로 `repetitions`라는 새로운 인스턴스 메서드를 추가합니다:
...
`repetitions(task:)` 메서드는 파라미터가 없고 반환 값이 없는 함수를 나타내는
`() -> Void` 타입의 인수를 가집니다.
...
이 예제는 Swift의 `Int` 타입에 정수 서브스크립트를 추가합니다.
...
이 예제는 `Int`에 새로운 중첩된 열거형을 추가합니다.
`Kind`라는 열거형은
특정 정수 표현에 대한 숫자의 종류를 나타냅니다.
명확하게 숫자가
음수, 0, 양수 인지 표현합니다.

이 예제는 정수에 대해 적절한 `Kind` 열거형 케이스를 반환하는
`kind`라는
새로운 연산 인스턴스 프로퍼티를 `Int`에 추가합니다.
