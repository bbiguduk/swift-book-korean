...
사용할 수 있는 독립적인 기능 블럭입니다.
Swift의 클로저는
다른 프로그래밍 언어에서
클로저, 익명 함수, 람다, 블럭과 유사합니다.
...
더 큰 함수의 부분으로
독립적인 코드 블럭을 이름 붙여 정의할 수 있는 편리한 수단입니다.
그러나 전체 선언과 이름 없이
함수와 유사한 구조의 짧은 형태로 작성하는 것이 때때로 유용합니다.
특히 함수나 메서드가 다른 함수를
하나 이상의 인수로 받을 때 그렇습니다.
...
항상 파라미터 타입과 반환 타입을 유추할 수 있습니다.
따라서 클로저가 함수나 메서드 인수로 사용될 때,
...
여기서 `sorted(by:)` 메서드의 인수는 함수 타입이므로,
...
선언에 클로저의 인수 목록을 생략할 수 있습니다.
축약 인수 이름의 타입은
함수 타입에서 추론되고,
사용한 축약 인수 중 가장 큰 번호가
클로저가 받는 인수의 개수를 결정합니다.
...
여기서 `$0`와 `$1`은 클로저의 첫 번째와 두 번째 `String` 인수를 나타냅니다.
`$1`이 축약 인수에서 가장 높은 숫자이므로,
클로저는 두 개의 인수가 있는 것으로 간주됩니다.
여기서 `sorted(by:)` 함수는
인수가 모두 문자열인 클로저로 기대하므로,
축약 인수 `$0`과 `$1`은 모두 타입 `String` 입니다.
...
Swift의 `String` 타입은
보다 큰 연산자(`>`)의
문자열 별 구현을 `String` 타입의 파라미터 두 개가 있는 메서드로 정의하고,
`Bool` 타입의 값을 반환합니다.
이것은 `sorted(by:)` 메서드에 필요한 메서드 타입과 정확하게 일치합니다.
따라서 보다 큰 연산자 자체를 인수로 전달하면,
...
단일 인수로 클로저 표현식을 가지는 `map(_:)` 메서드가 있습니다.
...
함수가 여러 개의 클로저를 인수로 받을 경우,
첫 번재 후행 클로저는 인수 레이블 없이 작성하고,
그 이후의 후행 클로저는 인수 레이블을 명시해야 합니다.
예를 들어
...
중첩 함수는 바깥 함수의 어떠한 인수도 캡처할 수 있고
...
*자동 클로저(autoclosure)*는 함수에 인수로 전달되는 표현식을 감싸기 위해
자동으로 생성되는 클로저입니다.
이 클로저는 인수를 가지지 않으며,
호출될 때,
내부에 감싸진 표현식의 값을 반환합니다.
이러한 구문상의 편의를 통해 명시적 클로저 대신에 일반 표현식을 작성하여
함수의 파라미터 주위의 중괄호를 생략할 수 있습니다.

자동 클로저를 가지는 함수를 *호출*하는 것은 일반적이지만
이러한 함수를 *구현*하는 것은 일반적이지 않습니다.
예를 들어
`assert(condition:message:file:line:)` 함수는
`condition`과 `message` 파라미터에 대한 자동 클로저를 가집니다;
`condition` 파라미터는 오직 디버그 빌드에서만 평가되고,
`message` 파라미터는 `condition`이 `false`에서만 평가됩니다.
...
고객의 이름을 반환하는 명시적 클로저를 인수를 가집니다.
아래 `serve(customer:)`의 버전은
같은 동작을 수행하지만, 명시적 클로저를 가지는 대신에
파라미터 타입에 `@autoclosure` 속성을 표기하여
자동 클로저를 가집니다.
이제 이 함수는
마치 `String` 타입 값을 인수로 받는 것처럼 호출할 수 있습니다.
`customerProvider` 파라미터의 타입은
