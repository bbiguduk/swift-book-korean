...
함께 저장되는 상수나 변수입니다.
이 두 프로퍼티는 정수 값 `0`으로 초기값이 설정되므로
`Int` 타입으로 추론됩니다.
...
구조체와 클래스 모두 새로운 인스턴스를 생성하기위해 초기화 구문을 사용합니다.
초기화 구문의 가장 단순한 형태는 `Resolution()`나 `VideoMode()`와 같이
...
### 구조체의 멤버별 초기화 구문 \(Memberwise Initializers for Structure Types\)

모든 구조체는 새로운 구조체 인스턴스의 멤버 프로퍼티를 초기화 할 때 사용할 수 있는
자동으로 생성되는 *멤버별 초기화 구문(memberwise intializer)*을 가집니다.
