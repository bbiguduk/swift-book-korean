...
> 스택은 컬렉션 관리에서
> "후입 선출(LIFO)" 접근 방식이 필요할 때 유용한 컬렉션 모델입니다.
...
`Int`의 실제 타입 대신에
`Element`라는 타입 파라미터를 사용합니다.
이 타입 파라미터는 구조체의 이름 바로 다음에
...
- `item`이라는 단일 파라미터를 가지는 `push(_:)` 메서드를 지정할 때
  `Element` 타입의 파라미터
- `pop()` 메서드에 의해 반환된 값이
  `Element` 타입의 값을 반환
...
제네릭 타입을 확장할 때,
확장의 정의에 타입 파라미터 목록을 제공하지 않습니다.
대신에 *기존* 타입 정의의 타입 파라미터 목록이
확장의 본문 내에서 사용 가능하고
기존 타입 파라미터 이름을
그대로 사용할 수 있습니다.
...
이 확장은 타입 파라미터 목록을 정의하지 않습니다.
대신에 `Element`라는 `Stack` 타입의 타입 파라미터 이름을
`topItem` 연산 프로퍼티의
...
타입 파라미터의 정의에 `Equatable` 타입 제약을 작성합니다:
...
`findIndex(of:in:)`의 단일 타입 파라미터는 "`T` 타입은 `Equatable` 프로토콜을 준수합니다"라는 의미로
`T: Equatable`로 작성됩니다.
...
다음은 위의 <doc:Generics#제네릭-타입-Generic-Types>에서
...
Swift는 `append(_:)` 메서드의 `item` 파라미터의 타입과
...
이번에는 타입 파라미터 `Element`는
`append(_:)` 메서드의 `item` 파라미터와
...
함수의 두 타입 파라미터에 대한 요구사항은 다음과 같습니다:
...
첫 번째와 두 번째 요구사항은 함수의 타입 파라미터 목록에 정의되고,
...
제네릭 `Item` 타입 파라미터에 대해 타입 제약조건을 추가한
제네릭 `where` 절을 포함합니다.
