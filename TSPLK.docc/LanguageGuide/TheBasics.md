...
*부호가 없는 정수(unsigned) (양수나 0)*가 있고
최대값과 최소값은 정수의 *크기*에 따라 결정됩니다
(값을 저장하는데 사용하는 비트수).
정수 타입은 이름에 크기와 부호가 포함되어 있습니다 ---
예를 들어 8비트 부호 없는 정수는 `UInt8` 타입이고,
32비트 부호 있는 정수는 `Int32` 타입입니다.
Swift에서의 타입과 같이 이 정수 타입은 대문자로 시작합니다.
대부분의 경우
정수 크기를 지정할 필요가 없다면,
아래 설명과 같이 `Int` 타입을 사용합니다.

정수 타입은 우리가 손으로 계산하는 산술 연산 방식처럼 동작하고
정수 연산은 근사값이 아닌 정확한 결과를 도출합니다.
이러한 특성은 정수가 개수를 세거나
정확한 값을 표현해야하는 계산에 적합합니다 ---
예를 들어
텍스트 파일에서 가장 긴 라인을 찾거나
게임에서 점수에 배율을 적용하거나
영수증의 가격을 합산할 때 유용합니다.

정수에는 소수 부분이 없지만,
소수 부분을 개수로 환산해서 세는 방식으로
소수를 포함한 값을 표현할 수 있습니다.
예를 들어
센트 단위로 세는 정수에서
$1.23을 `123`으로 저장해서 표현할 수 있습니다.
이러한 접근 방식은 소수점이 숫자에 고정된 위치에 있으므로
*고정 소수점 연산(fixed-point math)*라고 합니다.
위의 예제에서
숫자 `123`은 마지막 두 자리 앞에
소수점이 있다고 이해됩니다.

> Note:
> 금융이나 건설에서와 같은 곳의 계산이나
> 정밀한 결과가 요구되는 도메인에서는
> 해당 요구사항에 맞는
> 반올림이나 버림과 같은 동작이 구현된
> 특정 목적의 숫자 타입이 필요할 수 있습니다.
...
Swift는 정수에 다양한 크기의 타입이 있는 것처럼
다양한 크기를 지원하는
여러가지 부동소수점 타입을 제공합니다.
크기를 지정할 필요가 없다면, 기본적으로 `Double`을 사용합니다.
그렇지 않으면
`Float16`이나 `Float80`처럼
이름에 필요한 크기를 포함한 타입을 사용합니다.
부동소수점 연산에 일반적인 용어에 따라
`Float`는 32비트를 `Double`은 64비트를 사용합니다.
이 타입을 `Float32`나 `Float64`로 작성할 수도 있습니다.
예를 들어
그래픽 코드는 GPU의 가장 빠른 데이터 타입에 맞추기 위해 `Float`를 주로 사용합니다.
일부 부동소수점 타입은 특정 플랫폼에서나 지원하지만
`Float`와 `Double`은 모든 플랫폼에서 사용 가능합니다.

부동소수점 숫자는 매우 작은 숫자와
매우 큰 숫자를 다룰 수 있지만
범위 안의 모든 값을 표현할 수는 없습니다.
항상 정확한 결과를 나타내는
정수 계산과 다르게
부동소수점 연산은 표현할 수 있는 숫자에 가깝게 결과를 반올림합니다.
예를 들어
숫자 10,000을 `Float`로 저장하면,
표현할 수 있는 바로 다음 값은 10,000.001이고 ----
이 수 사이의 값은 둘 중 하나의 값으로 반올림됩니다.
숫자 사이의 간격도 가변적입니다;
큰 숫자 사이의 간격은 크고
작은 숫자의 간격은 작습니다.
예를 들어
0.001 다음의 `Float` 값은 0.0010000002이고,
10,000 다음의 값보다 간격이 더 작습니다.
...
부동소수점 숫자는
음의 0, 양의 무한대, 음의 무한대와 같은 값을 가지며
이것은 계산 중의 오버플로우와 언더플로우를 나타냅니다.
또한 숫자가 아닌 값(NaN)을 포함하고
이것은 0을 0으로 나누는 것처럼
유효하지 않거나 정의되지 않은 결과를 나타냅니다.
이것은 결과를 표현할 수 없을 때 프로그램을 중단하는
정수와는 다른 동작을 보여줍니다.

가능한 값 사이의 동일한 간격이 필요하거나
수행하는 계산이
정확한 결과를 요구하고
