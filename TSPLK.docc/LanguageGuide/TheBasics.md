...
타입 안정성은 개발 단계에서 가능한 빠르게 에러를 찾고 고칠 수 있게 도와줍니다.
...
*에러 처리(error handling)*는 프로그램 실행 중에
프로그램이 만날 수 있는 에러 조건에 대응하기 위해 사용됩니다.

값의 존재 유무를 사용하여
함수의 성공 또는 실패를 전달할 수 있는
옵셔널과 달리
에러 처리는 에러 원인을 판별하고
필요한 경우 에러를 프로그램의 다른 부분으로 전파할 수 있습니다.

함수가 에러 조건을 만나면, 그 함수는 에러를 *발생*시킵니다.
해당 함수의 호출자는 에러를 *포착*하고 적절하게 응답할 수 있습니다.
...
`do` 구문은 새로운 범위를 생성하여
에러가 하나 이상의 `catch` 절로 전파될 수 있도록 합니다.

다음은 에러 처리로
다양한 에러 조건에 대응하는 방법의 예입니다:
...
깨끗한 그릇을 사용할 수 없거나
재료가 없는 경우 에러가 발생할 것입니다.
`makeASandwich()` 함수는 에러를 발생할 수 있으므로,
`try` 표현식으로 래핑되어 호출합니다.
함수 호출을 `do` 구문으로 래핑하면
모든 에러는
`catch` 절로 전파됩니다.

에러가 발생하지 않으면 `eatASandwich()` 함수는 호출됩니다.
`SandwichError.outOfCleanDishes` 에러가 발생하면,
`washDishes()` 함수가 호출됩니다.
`SandwichError.missingIngredients` 에러가 발생하면,
...
어설션과 전제 조건은 코드 내에서
유용한 문서 형식이 됩니다.
위의 <doc:TheBasics#에러-처리-Error-Handling>와 다르게
