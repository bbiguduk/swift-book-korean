...
함수 파라미터는 <doc:Functions#함수-인수-라벨과-파라미터-이름-Function-Argument-Labels-and-Parameter-Names>에서 설명 했듯이
함수 본문 내에서 사용하는 이름과
함수 호출할 때 사용하는 인수 레이블을 모두 가질 수 있습니다.
메서드도 타입에 속한 함수이므로,
메서드 파라미터도 동일한 규칙을 따릅니다.
...
이 규칙의 주요 예외사항은 인스턴스 메서드의 파라미터 이름이
그 인스턴스의 프로퍼티 이름과 동일한 경우입니다.
이러한 경우 파라미터 이름이 더 우선되므로,
프로퍼티를 참조하려면 더 명확한 방식으로 접근해야 합니다.
파라미터 이름과 프로퍼티 이름이 충돌할 경우
`self` 프로퍼티를 사용하여 구분합니다.

다음 예제에서 `x`라는 메서드 파라미터와
...
`self` 접두사가 없으면,
Swift는 두 번 모두 `x`가 메서드의 파라미터 `x`를 가르키는 것으로 간주합니다.
...
위에서 본 `Point` 예제는 다음과 같은 방식으로 작성할 수도 있습니다:
...
이 예제는 세 가지 상태를 가지는 스위치를 나타내는 열거형을 정의합니다.
...
아래 예제는 게임 내에서 플레이어의 레벨이나 스테이지 진행사항을 추적하는
