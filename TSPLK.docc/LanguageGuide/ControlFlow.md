...
훨씬 더 강력합니다.
케이스들은 간격 일치, 튜플, 특정 타입으로의 캐스트를 포함하여
...
루프 안의 구문이 실행됩니다.
이 경우 루프는 `index`의 현재 값을 5배 하여 출력하는
하나의 구문만 포함합니다.
구문이 실행된 후에,
...
언더바 문자(`_`)는
각 값을 무시하고
루프의 각 반복동안 현재 값에 접근하는 것을 제공하지 않습니다.
...
`if` 표현식은 에러를 발생시키거나 반환하지 않는 `fatalError(_:file:line:)`와 같은 함수를
...
`.boiling` 에러를 발생시킵니다.
`if` 표현식은 에러를 발생할 수 있지만,
`try`를 전에 작성하지 않습니다.
에러에 대한 자세한 내용은 <doc:ErrorHandling>을 참고 바랍니다.
...
컴파일 때 에러가 발생합니다.
이 접근 방식은 한 케이스에서 다른 케이스로의 우발적인 실행을 방지하고
...
*제어 변경 구문(Control transfer statements)*은 한 코드에서 다른 코드로
제어를 변경하여 코드가 실행되는 순서를 변경합니다.
Swift는 다섯 개의 제어 변경 구문이 있습니다:
...
> Note: `switch` 케이스에 주석만 포함되어 있으면 컴파일 시 에러가 발생합니다.
...
값이 할당된 모든 변수나 상수는
`guard` 문이 존재하는
나머지 코드 블럭에서 사용할 수 있습니다.

조건이 충족되지 않으면,
`else` 안의 코드가 실행됩니다.
이 분기는 반드시 제어 흐름을 전환하여
`guard` 구문이 포함된 코드 블럭을 빠져나가야 합니다.
...
현재 범위의 마지막에 실행되는
코드 블럭을 작성하기 위해 `defer` 블럭을 사용합니다.
예를 들어:
...
같은 범위에 하나 이상의 `defer` 블럭을 작성하면,
첫 번째 `defer` 블럭은 마지막에 수행됩니다.
...
프로그램이 실행을 중지하면 ---
예를 들어, 런타임 에러 또는 충돌로 인해 ---
지연된 코드가 실행되지 않습니다.
그러나 에러가 발생한 후에는 지연된 코드가 실행됩니다;
`defer`를 에러 처리와 함께 사용하는 방법에 대한 자세한 내용은
...
코드 블럭을 조건부로 실행할 수 있습니다.
컴파일러는 해당 코드 블럭 내 API의 가용성을 확인할 때
