...
다음 코드는 `MediaItem` 의 두 개의 서브클래스를 정의합니다. 첫번째 서브클래스 `Movie` 는 영화 또는 필름에 대한 추가 정보를 캡슐화 합니다. 기본 `MediaItem` 클래스의 상위에 `director` 프로퍼티와 해당 초기화 구문을 추가합니다. 두번째 서브클래스 `Song` 은 기본 클래스의 상위에 `artist` 프로퍼티와 초기화 구문을 추가합니다:
