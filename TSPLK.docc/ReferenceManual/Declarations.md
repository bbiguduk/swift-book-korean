...
함수 내에서 기존값이 현재 범위에서 사용가능 하더라도 in-out 인수로 전달된 값은 접근하면 안됩니다. 기존값에 접근하는 것은 값에 대한 동시 접근이며 메모리 독점성을 위반합니다.
...
열거형의 케이스는 타입 멤버에 대해 프로토콜 요구사항을 충족할 수 있습니다. 특히, 연관값이 없는 열거형 케이스는 `Self` 타입의 get-only 타입 변수에 대해 프로토콜 요구사항을 충족하고 연관값이 있는 열거형 케이스는 파라미터와 인수 라벨이 케이스의 연관값과 일치하게 `Self` 를 반환하는 함수에 대해 프로토콜 요구사항을 충족합니다. 예를 들어:
