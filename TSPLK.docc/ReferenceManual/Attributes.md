...
이 속성을 함수나 메서드 선언에 적용하여
해당 정의를 클라이언트 모듈로 어떻게 내보낼 지 제어합니다.
내보낼 선언의 어떤 부분을 내보낼지 나타내기 위해
다음 중 하나의 인자를 포함합니다.

- `interface` 인자는
  호출 가능한 기호형태로
  인터페이스만 클라이언트에 내보내도록 지정합니다.
  정의(함수 본문)는
  인라인, 최적화, 어떤 목적에도 클라이언트에서 사용할 수 없습니다.
  이 인자를 사용하면 클라이언트로부터 구현을 숨길 수 있습니다.

- `implementation` 인자는
  정의(함수 본문)만 클라이언트에 내보내도록 지정합니다.
  이 함수에 대한 기호는 바이너리에 생성되지 않으며,
  클라이언트는 필요한 곳마다
  이 정의의 사본을 생성해야 합니다.
  이 인자를 사용해 Application Binary Interface(ABI)에 영향없이
  새로운 함수나 메서드를 도입할 수 있습니다.
