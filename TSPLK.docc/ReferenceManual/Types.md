...
두 번째 예시는
함수 `someFunction`의 파라미터 `a`는 타입 `Int`를 갖도록 지정합니다.
...
화살표(`->`)로 파라미터와 반환 타입을 구분하여 구성합니다:

```swift
(<#parameter type#>) -> <#return type#>
```

*파라미터 타입(parameter type)*은 콤마로 구분된 타입의 목록입니다.
...
함수 타입이 `() -> T`일 경우
(`T`는 모든 타입)
해당 파라미터는 `autoclosure` 속성을 적용하여
호출 시점에 암시적으로 클로저를 생성할 수 있습니다.
이것은 함수를 호출할 때
명시적으로 클로저를 작성할 필요없이
표현식의 평가를 연기하는
구문상 편리함을 제공합니다.
자동 클로저 함수 타입 파라미터의 예시는
<doc:Closures#자동-클로저-Autoclosures>를 참고바랍니다.

함수 타입의 *파라미터 타입*은 가변 파라미터(variadic parameters)를 가질 수 있습니다.
문법적으로
가변 파라미터는 `Int...`와 같이
기본 타입 이름 뒤에 점 3개(`...`)로 구성합니다. 가변 파라미터는 기본 타입 이름의
요소를 포함하는 배열로 처리합니다. 예를 들어 가변 파라미터 `Int...`는
`[Int]`로 처리합니다. 가변 파라미터를 사용하는 예시는
<doc:Functions#가변-파라미터-Variadic-Parameters>를 참고바랍니다.

in-out 파라미터(in-out parameter)를 지정하려면 `inout` 키워드를 파라미터 타입 앞에 붙여야 합니다.
가변 파라미터나 반환 타입에는 `inout` 키워드를 사용할 수 없습니다.
In-out 파라미터는 <doc:Functions#In-Out-파라미터-In-Out-Parameters>에 설명되어 있습니다.

함수 타입이 하나의 파라미터만 가지고 있고
파라미터 타입이 튜플 타입인 경우
...
함수 타입에서 오류 타입으로
연관 타입(associated type)이나 제네릭 타입 파라미터(generic type parameter)를 사용하면,
연관 타입이나 제네릭 타입 파라미터는
...
비탈출 함수(nonescaping function) 타입의 파라미터는
...
비탈출 함수 타입의 파라미터는
다른 비탈출 함수 파라미터 인자로 전달할 수도 없습니다.
이 제약은 Swift가
런타임이 아닌 컴파일 시
...
위의 코드에서
`takesTwoFunctions(first:second:)`의 두 파라미터는 모두 함수입니다.
두 파라미터 모두 `@escaping`으로 표시되지 않으므로
결과적으로 둘 다 비탈출입니다.

위의 예시에서 "Error"로 표시된 네 개의 함수 호출은
컴파일러 오류를 일으킵니다.
`first`와 `second` 파라미터는
비탈출 함수이므로,
다른 비탈출 함수 파라미터 인자로 전달할 수 없습니다.
반대로
"OK"로 표시된 두 개의 함수 호출은 컴파일러 오류를 발생시키지 않습니다.
이 함수 호출은
`external`이 `takesTwoFunctions(first:second:)`의 파라미터가 아니므로 제약에 위배되지 않습니다.

제약을 피해야 하는 경우 파라미터 중 하나를 탈출로 표시하거나,
`withoutActuallyEscaping(_:do:)` 함수를 이용하여
탈출 함수로 비탈출 함수 파라미터 중 하나를 임시로 변경해야 합니다.
