...
`is` 패턴은 런타임 시 해당 값의 타입이 `is` 패턴의 오른편에 지정한 타입과 일치하거나 해당 타입의 서브클래스가 일치하면 값으로 일치됩니다. `is` 패턴은 타입 캐스트를 동작하지만 반환된 타입을 버린다는 것에서 `is` 연산자와 유사하게 동작합니다.
...
`is` 와 `as` 패턴으로 값을 일치 시키기 위해 `switch` 구문을 사용하는 예제는 <doc:TypeCasting#Any-와-AnyObject-에-대한-타입-캐스팅-Type-Casting-for-Any-and-AnyObject> 을 참고 바랍니다.
